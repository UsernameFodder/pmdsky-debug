...
void InitMemArena(struct mem_arena *arena, struct iovec *mem,
                  struct mem_block *blocks, uint32_t max_blocks);
uint32_t MemAllocFlagsToBlockType(uint32_t alloc_flags);
int FindAvailableMemBlock(struct mem_arena *arena, uint32_t alloc_flags,
                          uint32_t len);
struct mem_block *SplitMemBlock(struct mem_arena *arena, uint32_t idx,
                                uint32_t alloc_flags, uint32_t len,
                                uint32_t user_flags);
void *MemAlloc(uint32_t len, uint32_t flags);
void MemFree(void *ptr);
struct mem_arena *MemArenaAlloc(struct mem_arena *parent_arena, uint32_t len,
                                uint32_t max_blocks, uint32_t flags);
...
bool UFixedPoint64CmpLt(int32_t x_upper, uint32_t x_lower, int32_t y_upper,
                        uint32_t y_lower);
...
void MultiplyFixedPoint64(struct fx64_16 *prod, struct fx64_16 *x,
                          struct fx64_16 *y);
void DivideFixedPoint64(struct fx64_16 *quotient, struct fx64_16 *dividend,
                        struct fx64_16 *divisor);
void UMultiplyFixedPoint64(struct fx64_16 *prod, struct fx64_16 *x,
                           struct fx64_16 *y);
...
void InitOamInfo(struct oam_info *oam_info, int max_num_objs,
                 int max_num_groups, int oam_base_address,
                 uint32_t mem_alloc_flags);
...
void AddObjToUngroupedOamObjs(struct oam_info *oam_info,
                              uint16_t *oam_attributes, int group);
...
uint32_t GetFileLengthInPackWithPackNb(enum pack_file_id pack_id,
                                       uint32_t file_index);
uint32_t LoadFileInPackWithPackId(enum pack_file_id pack_id,
                                  uint32_t file_index, void *output_buffer);
...
uint32_t GetFileLengthInPack(struct pack_file_opened *pack_file,
                             uint32_t file_index);
uint32_t LoadFileInPack(struct pack_file_opened *pack_file, void *output_buffer,
                        uint32_t file_index);
void GetDungeonResultMsg(union damage_source damage_source_or_result,
                         char *buffer, int buffer_size, undefined *param_4);
...
void InitItem(struct item *item, enum item_id item_id, uint16_t quantity,
              bool sticky);
...
bool IsItemAvailableInDungeonGroup(enum dungeon_id dungeon_id,
...
void GetItemNameFormatted(char *name_out, enum item_id item_id, bool flag1,
                          bool flag2);
...
void ApplyExclusiveItemStatBoosts(enum item_id item_id, uint8_t *atk_boost,
                                  uint8_t *sp_atk_boost, uint8_t *def_boost,
                                  uint8_t *sp_def_boost);
void SetExclusiveItemEffect(uint32_t *effect_flags,
                            enum exclusive_item_effect_id effect_id);
bool ExclusiveItemEffectFlagTest(uint32_t *effect_flags,
                                 enum exclusive_item_effect_id effect_id);
bool IsExclusiveItemIdForMonster(enum item_id item_id,
                                 enum monster_id monster_id, enum type_id type1,
                                 enum type_id type2);
bool IsExclusiveItemForMonster(struct item *item, enum monster_id monster_id,
                               enum type_id type1, enum type_id type2);
enum item_id
BagHasExclusiveItemTypeForMonster(enum exclusive_item_effect_id effect_id,
                                  enum monster_id monster_id,
                                  enum type_id type1, enum type_id type2);
bool GetExclusiveItemForMonsterFromBag(struct item *item,
                                       enum exclusive_item_effect_id effect_id,
                                       enum monster_id monster_id,
                                       enum type_id type1, enum type_id type2);
...
struct move_target_and_range GetMoveTargetAndRange(struct move *move,
                                                   bool is_ai);
...
void CopyMovesetFromStream(struct bitstream *bitstream,
                           struct ground_move *dst);
...
void PlayBgmByIdVolumeVeneer(enum music_id music_id, undefined param_2,
                             int volume);
...
void FillCopyToFlatVramCommand(struct copy_to_obj_vram_order *order, void *dst,
                               void *src, uint32_t len_output,
                               enum copy_to_obj_vram_order_type copy_type,
                               uint16_t interleave_with);
void ExecuteCopyToFlatVramCommand(struct copy_to_obj_vram_order *order);
uint32_t
DecodeFragmentByteAssemblyTable(struct wan_fragment_bytes_assembly_entry *entry,
                                void *dst);
int LoadObjPalette(struct obj_graphics_control *obj_graphics_control,
                   struct palette_init_info *palette_init_info,
                   uint8_t palette_num);
int AddSimpleObjToOam(struct obj_graphics_control *obj_graphics_control,
                      uint16_t *obj, int group);
void GroupOamAttributesWrapper(
    struct obj_graphics_control *obj_graphics_control);
void CopyAttributesToOamWrapper(
    struct obj_graphics_control *obj_graphics_control);
void ChangeSimpleObjTexture(struct obj_graphics_control *obj_graphics_control,
                            undefined4 *src, uint16_t oam_tile_num,
                            uint16_t texture_size, bool extended_palette,
...
void CopyAndInterleaveWrapper(uint16_t *dst, uint16_t *src, uint32_t len,
                              uint8_t val);
...
void SetSpriteIdForAnimationControl(struct animation_control *anim_ctrl,
                                    uint16_t sprite_id);
void SetAnimationForAnimationControlInternal(
    struct animation_control *anim_ctrl, struct wan_header *wan_header,
    int animation_group_id, int animation_id, int unk1, int low_palette_pos,
    int unk2, int unk3, int palette_bank);
void SetAnimationForAnimationControl(struct animation_control *anim_ctrl,
                                     int animation_key,
                                     enum direction_id direction, int unk1,
                                     int low_palette_pos, int unk2, int unk3,
                                     int unk4);
struct wan_header *
GetWanForAnimationControl(struct animation_control *anim_ctrl);
void SetAndPlayAnimationForAnimationControl(struct animation_control *anim_ctrl,
                                            int animation_key,
                                            enum direction_id direction,
                                            int unk1, int low_palette_pos,
                                            int unk2, int unk3, int unk4);
void SwitchAnimationControlToNextFrame(struct animation_control *anim_ctrl);
void LoadAnimationFrameAndIncrementInAnimationControl(
...
uint32_t
AnimationControlGetAllocForMaxFrame(struct animation_control *anim_ctrl);
...
int GetLoadedWanTableEntry(struct wan_table *wan_table,
                           enum pack_file_id pack_id, uint16_t file_index);
void InitWanTable(struct wan_table *wan_table);
int LoadWanTableEntry(struct wan_table *wan_table, const char *path,
                      uint32_t flags);
int LoadWanTableEntryFromPack(struct wan_table *wan_table,
                              enum pack_file_id pack_id, uint16_t file_index,
                              uint32_t alloc_flags, bool is_compressed);
int LoadWanTableEntryFromPackUseProvidedMemory(struct wan_table *wan_table,
                                               enum pack_file_id pack_id,
                                               uint16_t file_index,
                                               void *sprite_storage,
                                               bool is_compressed);
int ReplaceWanFromBinFile(struct wan_table *wan_table, int wan_id,
                          enum pack_file_id pack_id, uint16_t file_index,
                          bool compressed);
void DeleteWanTableEntryVeneer(struct wan_table *wan_table, int wan_id);
bool WanHasAnimationGroup(struct wan_header *wan_header,
...
void LoadWteFromRom(struct wte_handle *handle, const char *path,
                    uint32_t flags);
void LoadWteFromFileDirectory(struct wte_handle *handle,
                              enum pack_file_id pack_id, uint16_t file_index,
                              uint32_t malloc_flags);
...
void ProcessWte(undefined *wte_header_ptr, undefined4 texture_vram_offset,
                undefined4 pal_vram_offset_upper,
...
uint32_t
PlanCopyTextureToTextureVram(struct delayed_texture_vram_container *container,
                             void *image_pointer, uint32_t dest_vram_offset,
                             uint32_t image_length,
                             enum delayed_texture_vram_order_type order_type);
void PerformPlannedTextureVramTransfer(
    struct delayed_texture_vram_container *container);
void GeomSetTexImageParam(int texture_format,
                          int texture_coordinates_transformation_modes,
                          int texture_s_size, int texture_t_size,
                          int repeat_s_t, int flip_s_t, bool color_0,
                          int vram_offset);
...
void FillPaletteInitInfo(struct palette_init_info *palette_init_info,
                         struct rgba *palette_bytes, uint16_t palette_mode,
                         uint16_t nb_colors_or_palettes,
                         uint16_t ext_palette_upper, int8_t palette_num_custom);
int DecompressAtNormalVeneer(undefined *addr_decomp, int expected_size,
                             undefined *at_ptr);
int DecompressAtNormal(undefined *addr_decomp, int expected_size,
                       undefined *at_ptr);
int DecompressAtHalf(undefined *addr_decomp, int expected_size,
                     undefined *at_ptr, int high_nibble);
int DecompressAtFromMemoryPointerVeneer(undefined *addr_decomp,
                                        int expected_size, undefined *at_ptr);
int DecompressAtFromMemoryPointer(undefined *addr_decomp, int expected_size,
                                  undefined *at_ptr);
...
void SubstitutePlaceholderItemTags(uint8_t tag_id, struct item *item,
                                   undefined4 *param_3);
...
int GetTalkLine(int personality_idx, enum talk_type talk_type,
                int restrictions);
bool IsAOrBPressed(void);
void DrawTextInWindow(int window_id, int x, int y, char *string);
void AppendStandardStringToMission(undefined4 param_1, undefined4 param_2,
                                   undefined4 param_3, int string_id);
...
int GetPaletteBaseAddress(undefined4 pal_vram_offset_upper,
                          undefined4 pal_vram_offset_lower);
...
int PrintSpecialEpisodeClearMark(enum special_episode_type special_episode_type,
                                 int x, int y, undefined param_4);
...
int CreateParentMenuFromStringIds(struct window_params *params,
...
int CreateParentMenuWrapper(struct window_params *params,
                            struct window_flags flags,
                            struct window_extra_info *window_extra_info,
                            struct simple_menu_ptr_item *menu_items);
int CreateParentMenuInternal(struct window_params *params,
                             struct window_flags flags,
                             struct window_extra_info *window_extra_info,
...
int CreateSimpleMenuFromStringIds(struct window_params *params,
                                  struct window_flags flags,
                                  struct window_extra_info *window_extra_info,
                                  struct simple_menu_id_item *menu_items,
                                  int n_items);
...
int CreateSimpleMenuInternal(struct window_params *params,
                             struct window_flags flags,
                             struct window_extra_info *window_extra_info,
                             struct simple_menu_item *menu_items, int n_items);
...
int CreateAdvancedMenu(struct window_params *params, struct window_flags flags,
                       struct window_extra_info *window_extra_info,
                       advanced_menu_entry_fn_t entry_fn, int n_options,
                       int n_opt_per_page);
...
int CreateCollectionMenu(struct window_params *params,
                         struct window_flags flags,
                         struct window_extra_info *window_extra_info,
                         unk_collection_menu_fn_t param_4, undefined4 param_5,
...
int CreateOptionsMenu(struct window_params *params, struct window_flags flags,
                      struct window_extra_info *window_extra_info,
                      struct options_menu_id_item *menu_items, int n_items,
                      int *option_states);
...
int CreateDebugMenu(struct window_params *params, struct window_flags flags,
                    struct window_extra_info *window_extra_info,
                    uint16_t *menu_item_string_ids, int n_items,
                    undefined *param_6);
...
int CreateScrollBoxSingle(struct window_params *params,
                          struct window_flags flags,
                          struct window_extra_info *window_extra_info,
                          uint16_t string_id1, struct preprocessor_args *args1,
                          uint16_t string_id2, struct preprocessor_args *args2);
int CreateScrollBoxMulti(struct window_params *params,
                         struct window_flags flags,
                         struct window_extra_info *window_extra_info,
                         int n_strings, uint16_t *string_ids1,
                         struct preprocessor_args *args1, uint16_t *string_ids2,
...
void ShowStringIdInDialogueBox(int window_id, struct preprocessor_flags flags,
                               int string_id, struct preprocessor_args *args);
void ShowStringInDialogueBox(int window_id, struct preprocessor_flags flags,
                             char *string, struct preprocessor_args *args);
...
int CreateTextBoxWithArg(struct window_params *params,
                         text_box_callback_with_arg_fn_t cb, void *cb_arg);
...
bool AddMessageToAlertBox(int window_id, struct preprocessor_flags flags,
                          char *message,
                          struct preprocessor_args *preprocessor_args,
                          bool is_new_group);
bool IsAlertBoxActive(int window_id);
void UpdateAlertBox(struct window *window);
int CreateAdvancedTextBox(struct window_params *params,
                          struct window_flags flags,
                          struct window_extra_info *extra_info,
                          text_box_callback_fn_t cb, int n_items);
int CreateAdvancedTextBoxWithArg(struct window_params *params,
                                 struct window_flags flags,
                                 struct window_extra_info *extra_info,
                                 text_box_callback_with_arg_fn_t cb,
                                 void *cb_arg, int n_items);
struct advanced_text_box *CreateAdvancedTextBoxInternal(
    struct window_params *params, struct window_flags flags,
    struct window_extra_info *extra_info, int n_items, int n_items_per_page);
...
void SetUnkAdvancedTextBoxWindowFn(int window_id,
                                   unk_advanced_text_box_window_fn_t fn);
void UpdateAdvancedTextBox(struct window *window);
void PlayAdvancedTextBoxInputSound(int window_id, int index);
int CreateTeamSelectionMenu(struct window_params *params,
                            struct window_flags flags,
                            struct window_extra_info *window_extra_info,
                            team_selection_menu_get_item_fn_t get_item,
...
int CalcMenuHeightDiv8(struct window_flags flags,
                       struct window_extra_info *extra_info, int *n_options,
                       int *n_opt_per_page);
void InitWindowInput(struct window_input_ctx *input_ctx,
                     struct window_flags flags,
                     struct window_extra_info *window_extra_info,
                     struct window_rectangle *rect, int n_items,
...
void InitInventoryMenuInput(struct inventory_menu_input_ctx *input_ctx,
                            struct window_flags flags,
                            struct window_extra_info *window_extra_info,
                            struct window_rectangle *rect, int n_items,
                            int n_items_per_page, undefined param_7);
...
bool OverlayLoadEntriesEqual(struct overlay_load_entry *entry1,
                             struct overlay_load_entry *entry2);
void FreeActiveMenu(void);
bool InitMenu(struct overlay_load_entry *entry);
bool InitMenuWithWindowExtraInfo(struct overlay_load_entry *entry,
                                 bool init_extra_info,
                                 struct window_extra_info *window_extra_info);
bool CopyMenuControlWindowExtraInfo(
    struct window_extra_info *window_extra_info);
...
void AssignSpecialEpisodePc(int team_member_id,
                            struct special_episode_pc *special_episode_pc);
...
undefined4 ReadSaveHeader(undefined4 *param_1, undefined4 param_2,
...
int LoadScriptVariableValueAtIndex(void *local_var_vals, enum script_var_id id,
                                   int idx);
void SaveScriptVariableValue(void *local_var_vals, enum script_var_id id,
                             int val);
void SaveScriptVariableValueAtIndex(void *local_var_vals, enum script_var_id id,
                                    int idx, int val);
int LoadScriptVariableValueSum(void *local_var_vals, enum script_var_id id);
void LoadScriptVariableValueBytes(enum script_var_id id, void *dest,
                                  uint32_t n);
void LoadScriptVariableValueString(enum script_var_id id, void *dest,
                                   uint8_t n);
void SaveScriptVariableValueBytes(enum script_var_id id, void *src, uint32_t n);
bool ScriptVariablesEqual(void *local_var_vals, enum script_var_id id1,
                          enum script_var_id id2);
int CalcScriptVariables(int val0, int val1,
                        enum script_calc_operation operation);
bool CompareScriptVariables(int val0, int val1,
                            enum script_compare_operation operation);
int CalcScriptVariablesVeneer(int val0, int val1,
                              enum script_calc_operation operation);
void CalcAndUpdateScriptVarWithOtherValue(void *local_var_vals,
                                          enum script_var_id id, int other_val,
                                          enum script_calc_operation operation);
void CalcAndUpdateScriptVarWithOtherScriptVar(
    void *local_var_vals, enum script_var_id id1, enum script_var_id id2,
    enum script_calc_operation operation);
bool CompareScriptVariablesVeneer(int val0, int val1,
                                  enum script_compare_operation operation);
bool LoadAndCompareScriptVarAndValue(void *local_var_vals,
                                     enum script_var_id id, int other_val,
                                     enum script_compare_operation operation);
bool LoadAndCompareScriptVars(void *local_var_vals, enum script_var_id id1,
                              enum script_var_id id2,
                              enum script_compare_operation operation);
...
void SetSpecialEpisodeOpen(enum special_episode_type special_episode_type,
                           bool episode_open);
bool IsSpecialEpisodeOpenMismatch(void);
bool IsSpecialEpisodeOpenOld(enum special_episode_type special_episode_type);
void SetSpecialEpisodeOpenOld(enum special_episode_type special_episode_type,
                              bool episode_open);
...
void InitPortraitParamsWithMonsterId(struct portrait_params *portrait,
                                     enum monster_id monster_id);
void SetPortraitEmotion(struct portrait_params *portrait,
...
void InitDungeonInit(struct dungeon_init *dungeon_init_data,
...
void DungeonFloorToGroupFloor(
    struct dungeon_group_and_group_floor *out_group_data,
...
bool IsForbiddenFloor(undefined *dungeon_info, undefined4 param_2,
                      undefined4 param_3, undefined4 param_4);
...
void InitializeTeamMemberFromMentry(struct team_member *team_member,
                                    struct ground_monster *ground_monster,
                                    int32_t slot_index);
void GetLvlUpEntry(struct level_up_entry *level_up_entry,
                   enum monster_id monster_id, int level);
...
enum monster_id
GetBaseFormBurmyWormadamShellosGastrodonCherrim(enum monster_id monster_id);
...
void SetMonsterId(struct monster_spawn_entry *monster_spawn,
                  enum monster_id monster_id);
...
void ApplyLevelUpBoostsToGroundMonster(struct ground_monster *ground_monster,
                                       int level, bool flag);
...
bool GendersEqualNotGenderless(enum monster_id monster1,
                               enum monster_id monster2);
bool GendersNotEqualNotGenderless(enum monster_id monster1,
                                  enum monster_id monster2);
...
void GetRecoloredGroundMonsterName(char *out, struct ground_monster *monster,
                                   char color_symbol);
...
int GetLearnableIqSkills(struct iq_skill_id_8 *out_iq_skill_id,
                         enum monster_id monster_id, int monster_iq);
...
void EnableAllLearnableIqSkills(uint32_t *iq_skills_flags,
                                enum monster_id monster_id, int monster_iq);
...
void CopyMonsterToStream(struct bitstream *stream,
                         struct ground_monster *monster);
void CopyMonsterFromStream(struct bitstream *stream,
                           struct ground_monster *monster);
...
bool CheckMonsterForMissionType(enum mission_type type,
                                union mission_subtype *subtype,
                                enum monster_id target, bool allow_no_monster);
bool CheckItemForMissionType(enum mission_type type,
                             union mission_subtype *subtype, enum item_id item);
enum mission_generation_result GenerateMission(undefined *param_1,
                                               struct mission *mission_data);
int CountAndPopulateValidMissionTableMonsters(enum monster_id *monster_table,
                                              undefined4 *range);
bool IsMissionTypeSpecialEpisode(struct mission *mission);
bool DoesMissionHaveTypeAndSubtype(struct mission *mission,
                                   struct mission_type_8 *type,
                                   union mission_subtype *subtype);
bool AlreadyHasSimilarMission(struct mission_type_8 *type,
                              union mission_subtype *subtype);
...
bool WasMissionCompletedToday(
    struct mission_result_and_client *result_and_client,
...
enum fixed_room_id GetMissionSpecificFixedRoom(enum mission_type type,
                                               union mission_subtype subtype);
int8_t SelectRandomFixedRoomInRange(struct fixed_room_id_8 *fixed_rooms,
                                    int max);
void ReadRescueBinFile(void);
void GenerateMissionDetailsStruct(struct mission *mission,
...
bool ValidateLegendaryChallengeMission(
    struct mission_template *valid_mission_info, struct mission *mission);
struct mission_template *MatchMissionTemplateToMission(undefined4 table_index,
                                                       struct mission *mission);
void AppendMissionTitle(char *main_buffer, char *temp_buffer,
                        struct preprocessor_args *args,
                        struct mission_details *details);
...
void AppendMissionSummary(char *main_buffer, char *temp_buffer, int window_id,
                          int y_offset, struct preprocessor_args *args,
                          struct mission_details *details);
void MakeMissionDetails(struct mission_details *mission_details, char *buffer,
                        undefined param_3);
void MakeSpecialEpisodeMissionDetails(char *buffer, undefined param_2);
undefined PrintWonderMailSkyCode(undefined param_1, undefined param_2,
                                 undefined param_3, undefined param_4);
...
void AppendMissionDungeonLocation(struct mission_details *mission_details,
...
bool CanMonsterBeUsedForMission(enum monster_id monster_id,
                                bool check_story_banned);
...
enum mission_rank
GetMissionRankWithCapAndModifiers(struct dungeon_floor_pair *dungeon_floor_pair,
                                  enum mission_type type);
enum mission_rank GetMissionRankWithCapAndModifiersAndCap(
    struct dungeon_floor_pair *dungeon_floor_pair, enum mission_type type);
bool CanSendItem(enum item_id item_id, bool to_sky);
void InitMissionReward(struct mission *mission,
                       struct mission_reward_data *reward_data,
                       undefined4 param_3, undefined4 param_4);
void RollRandomItemReward(undefined4 param_1, undefined4 param_2,
...
void SetRandomRequestNpcs1And2(enum script_entity_id actor_id_1,
                               enum script_entity_id actor_id_2);
void SetRandomRequestNpc03KindVar(enum monster_id id);
void SetAllEventNpcs(enum script_entity_id actor_id_1,
                     enum script_entity_id actor_id_2,
                     enum script_entity_id actor_id_3,
