...
// Really, this appears to be the FSFile struct from NitroSDK
struct dse_file_wrapper {
    uint8_t data[72];
};
ASSERT_SIZE(struct dse_file_wrapper, 72);
...
struct dse_driver_work {
    ...
    uint8_t driver_thread[192]; // Seems to be the OSThread structure from NitroSDK
    void* driver_stack;
    ...
    uint8_t sample_loader_thread[192]; // Again, OSThread
    void* sample_loader_thread_stack;
    struct dse_wavebank* loading_bank;
    void* loading_bank_pcm_data;
    // There's more, but nothing labeled.
};
ASSERT_SIZE(struct dse_driver_work, 7872);
