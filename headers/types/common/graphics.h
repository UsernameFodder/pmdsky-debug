...
struct palette_data {
    ...
    struct palette_data* next_palette;
};
ASSERT_SIZE(struct palette_data, 40);

struct ungrouped_oam_obj {
...
ASSERT_SIZE(struct ungrouped_oam_obj, 8);

struct grouped_oam_obj {
    uint16_t oam_attributes[3]; // 0x0: See https://problemkaputt.de/gbatek.htm#lcdobjoamattributes
    undefined2 field_0x6;
};
ASSERT_SIZE(struct grouped_oam_obj, 8);
