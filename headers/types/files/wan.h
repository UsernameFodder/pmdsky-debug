...
struct wan_animation_header {
    ...
    uint16_t is_256_color_alt; // appear to always be 0, but the game seems to still decode it the
                               // image header is missing for some reason
...
// An animation Group is a set of animation, either a single or 8 for the eight possible orientation
// of a monster. An animation is itself made of frame.
struct wan_animation_group {
    struct wan_animation_frame***
        pnt; // first level is the animation groups, second is animations, third is frame pointer
    uint16_t len;
