/// Basic object types used by the resymgen YAML format
...
// For returning either a Once iterator or some other iterator, while still allowing static dispatch
pub enum OrOnce<T, I>
where
    I: Iterator<Item = T>,
{
...
// For returning either an Empty iterator or some other iterator, while still allowing static dispatch
...
pub struct OrdString {
    #[serde(skip)]
    ord: u64, // Used to dynamically control the sort order
...
/// Compare against just the value, irrespective of the ordinal
...
/// Compare against just the name, irrespective of the ordinal
...
// This is a more general implementation that lets you pass in keys
// that are coercible into a Version, but it requires runtime iteration >:(
// Should probably look into how to implement this with macros...
// impl<T, V, const N: usize> From<[(V, T); N]> for VersionDep<T>
// where
//     V: Into<Version>,
// {
//     fn from(arr: [(V, T); N]) -> Self {
//         // Transform the array elements, then collect it back to a
//         // Vec before converting it back into an array.
//         let converted: [(Version, T); N] = std::array::IntoIter::new(arr)
//             .map(|pair| (pair.0.into(), pair.1))
//             .collect::<Vec<_>>()
//             .try_into()
//             .unwrap_or_else(|v| panic!("Array conversion failed"));
//         VersionDep(BTreeMap::from(converted))
//     }
// }
